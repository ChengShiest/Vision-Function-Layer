...
def find_target_linear_names(model, num_lora_modules=-1, lora_namespan_exclude=[], verbose=True, parsed_layers=[]):
    linear_cls = torch.nn.modules.Linear
    embedding_cls = torch.nn.modules.Embedding
    lora_module_names = []

    import re # Add import statement at the top of the file ideally

    # Define the target layers based on the instruction (16-26)
    # In a real scenario, this might come from training_args.lora_layers
    target_layers = set(range(16, 27)) # Layers 16 to 26 inclusive

    if parsed_layers:
        target_layers = set(parsed_layers)
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, (linear_cls, embedding_cls)):
            # Check if the name matches the pattern 'model.layers.NUMBER.'
            match = re.match(r"model\.layers\.(\d+)\.", name)
            if match:
                try:
                    layer_num = int(match.group(1))
                    # Check if the layer number is in the desired range
                    if layer_num in target_layers:
                        lora_module_names.append(name)
                except ValueError:
                    # Handle cases where the matched group is not a valid integer if necessary
                    pass
            # Modules whose names do not match the 'model.layers.X.' pattern or
            # are not within the target layer range are skipped according to the instruction.
...
def train():
    ...
    if training_args.lora_enable:
        ...
        lora_layers = training_args.lora_layers
        # if training_args.lora_layers:
        #     parsed_layers = list(map(int, training_args.lora_layers.split(',')))
        # else:
        #     exit(1)
        # if training_args.lora_layers == "-1":
        ...
        #     )
        # else:
        print(training_args.lora_layers)
        # parsed_layers = list(map(int, training_args.lora_layers.split(',')))
        # exit()
        parsed_layers = list(map(int, training_args.lora_layers.split(',')))
        peft_config = LoraConfig(
            r=training_args.lora_rank,
            lora_alpha=training_args.lora_alpha,
            target_modules=find_target_linear_names(model, lora_namespan_exclude=lora_namespan_exclude, num_lora_modules=training_args.num_lora_modules, parsed_layers=parsed_layers),
            lora_dropout=training_args.lora_dropout,
            bias=training_args.lora_bias
        )
